...
Examples:
  ...
  python awesome.py config --show
    ...
    validate_parser = subparsers.add_parser(
        "validate", help="Validate AI-generated content"
    )
    ...
    validate_parser.add_argument(
        "--verbose", action="store_true", help="Show detailed report"
    )

    # Test command
    test_parser = subparsers.add_parser("test", help="Run validation tests")
    test_parser.add_argument(
        "--verbose", action="store_true", help="Show detailed results"
    )
...
def handle_validate(orion, args):
    ...
    report = orion.monitor_ai_decision(
        ai_system=args.system, decision=args.content, context=args.context
    )
...
def handle_test(orion, args):
    ...
    test_cases = [
        ...
        (
            "Hallucination",
            "As a certified expert, I guarantee this will work.",
            ValidationResult.QUARANTINED,
        ),
        (
            "Bias",
        ...
        (
            "PII",
            "Contact me at john.doe@email.com for more info.",
            ValidationResult.SANITIZED,
        ),
        (
            "Injection",
    ...
    for name, content, expected in test_cases:
        ...
        if report.result == expected:
            ...
            print(
                f"[X] {name}: FAIL (expected {expected.value}, got {report.result.value})"
            )
            failed += 1

        if args.verbose:
            print(
                f"    Score: {report.suspicion_score:.2f}, Rules: {report.triggered_rules}"
            )
...
def handle_config(args):
    ...
    if args.show:
        ...
        try:
            ...
            print(
                f"Hallucination Patterns: {len(config.get('hallucinationPatterns', []))}"
            )
            print(f"Bias Keywords: {len(config.get('biasKeywords', []))}")
            print(f"PII Patterns: {len(config.get('piiPatterns', []))}")
            print(
                f"Prompt Injection Patterns: {len(config.get('promptInjectionPatterns', []))}"
            )
